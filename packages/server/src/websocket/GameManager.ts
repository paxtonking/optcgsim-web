...
export class GameManager {
  ...
  private async loadPlayerDeck(playerId: string, deckId: string) {
    ...
    if (deck.userId !== playerId) {
      console.warn(`[GameManager] Deck ${deckId} belongs to ${deck.userId}, not ${playerId} — allowing for game start`);
    }
      ...
      if (isDefensivePhase) {
        ...
        if (state.activePlayerId === socket.userId) {
          ...
          if (!allowedActiveActions.includes(action.type as ActionType)) {
            if (callback) callback({ success: false, error: 'Waiting for opponent' });
            return;
          }
        }
